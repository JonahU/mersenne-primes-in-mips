...
// Assume a >= b, and that both a and b are positive
Bigint sub_big(Bigint a, Bigint b)
{
	...
	for(int i=0; i<b.n; i++) {
		if (a.digits[i] > b.digits[i] + carry) {
			...
			carry = 0;
		} else if (a.digits[i] < b.digits[i] + carry) {
			c.digits[i] = 10 + a.digits[i] - b.digits[i] - carry;
			carry = 1;
		} else { // a.digits[i] == b.digits[i] + carry
			c.digits[i] = 0;
			carry = 0;
		}
	...
	if (carry > 0) {
		c.digits[b.n] = a.digits[b.n] - carry;
	}
